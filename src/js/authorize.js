...
async function fastAuthCheck() {
  try {
    const response = await fetch("https://api.auth.erzen.xyz/v1/auth/info", {
      method: "GET",
      credentials: "include",
    });
    return response.ok;
...
async function loadConsentScreen() {
  try {
    ...
    const response = await fetch(
      `https://api.auth.erzen.xyz/oauth/consent?client_id=${clientId}&scope=${scope}`,
      {
        method: "GET",
        credentials: "include",
        headers: {
    ...
    if (!data.grantedScopes) {
      ...
      const authResponse = await fetch(
        `https://api.auth.erzen.xyz/oauth/authorize?client_id=${clientId}&redirect_uri=${redirectUri}&scope=${scope}&state=${state}&response_type=${responseType}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
...
document.getElementById("authorizeBtn").addEventListener("click", async () => {
  try {
    ...
    await fetch("https://api.auth.erzen.xyz/oauth/consent", {
      method: "POST",
      credentials: "include",
      headers: {
