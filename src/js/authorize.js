...
async function loadConsentScreen() {
  try {
    ...
    const response = await fetch(
      `https://apis.erzen.tk/oauth/consent?client_id=${clientId}&scope=${scope}`,
      {
        method: "GET",
        credentials: "include",
        headers: {
    ...
    if (!data.grantedScopes) {
      ...
      const authResponse = await fetch(
        `https://apis.erzen.tk/oauth/authorize?client_id=${clientId}&redirect_uri=${redirectUri}&scope=${scope}&state=${state}&response_type=${responseType}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
...
async function refreshToken() {
  ...
  try {
    ...
    localStorage.setItem("expires_at", new Date().getTime() + 9.5 * 60 * 1000);
...
document.getElementById("authorizeBtn").addEventListener("click", async () => {
  try {
    ...
    await fetch("https://apis.erzen.tk/oauth/consent", {
      method: "POST",
      credentials: "include",
      headers: {
        "Content-Type": "application/json",
